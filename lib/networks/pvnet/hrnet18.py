...
class Resnet18(nn.Module):
    def __init__(self, ver_dim, seg_dim, fcdim=256, s8dim=128, s4dim=64, s2dim=32, raw_dim=32):
        ...
        with open('lib/networks/pvnet/hrnet_config.yaml','r') as fp:
            hrnet_cfg = yaml.safe_load(fp)
        hrnet_cfg = CN(hrnet_cfg)
        
        hrnet18 = get_seg_model(hrnet_cfg)
        self.hrnet18 = hrnet18
...
#         self.single_stage = SimplePnPNet(4, 64*self.N_grid*self.N_grid).cuda()
    ...
    def forward(self, x, seg_gt=torch.zeros(1), feature_alignment=False):       
        x2s, x4s = self.hrnet18(x)

        fm=self.up4sto2s(x4s)
...
#                 bbox_scaled = (bbox / 4).astype(np.int)
